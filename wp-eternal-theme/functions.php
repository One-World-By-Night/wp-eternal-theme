...
function add_tag_userid_fields($taxonomy)
{
    $current_user_id = get_current_user_id();
    echo '<div class="form-field">';
    echo '<input type="hidden" name="user_id_text_tag" id="user_id_text_tag" value="' . $current_user_id . '" />';
    echo '</div>';
}
add_action('doc_tag_add_form_fields', 'add_tag_userid_fields');
...
function rudr_save_tag_userid_fields($term_id)
{
    update_term_meta(
        $term_id,
        'user_id_text_tag',
        sanitize_text_field($_POST['user_id_text_tag'])
    );
}
add_action('created_doc_tag', 'rudr_save_tag_userid_fields');
...
function filter_tex($query)
{
    ...
    /****** Tag *****/
    if (is_admin() && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'doc_tag') {
        $current_user = wp_get_current_user();
        if (in_array('author', $current_user->roles)) {
            $user_id = $current_user->ID;
            $meta_query = array(
...
function set_user_id_in_post_callback($post)
{
    ...
    echo '<label style="display:none;" for="my_custom_meta_field">User Id:</label>';
    if ($value) {
        echo '<input type="text" readonly id="my_custom_meta_field" name="post_created_by_user_id" value="' . esc_attr($value) . '" style="width:100%; display:none;" />';
    } else {
        $current_user = wp_get_current_user();
        $user_id = $current_user->ID;
        echo '<input type="text" readonly id="my_custom_meta_field" name="post_created_by_user_id" value="' . esc_attr($user_id) . '" style="width:100%;" />';
    }
}
...
add_filter('views_edit-docs', 'customize_subsubsub_counts_by_meta');
function customize_subsubsub_counts_by_meta($views)
{
    $current_user = wp_get_current_user();
    if (in_array('author', $current_user->roles)) {
        global $wpdb;

        // Define meta key, meta value, and post type
        $meta_key = 'post_created_by_user_id';
        $meta_value = get_current_user_id();
        $post_type = 'docs';

        // Query for all posts count
        ...
        // Query for published posts count
        ...
        // Update the counts in the views
        if (isset($views['all'])) {
        ...
        if (isset($views['publish'])) { // Correct status key: 'publish' not 'published'
            $views['publish'] = preg_replace(
                '/\(\d+\)/',
                '(' . intval($all_published) . ')',
                $views['publish']
            );
        }
...
{
    if (is_admin() && isset($_GET['page']) && $_GET['page'] == 'betterdocs-admin') {
        wp_redirect(admin_url('edit.php?post_type=docs&bdocs_view=classic'));
        exit;
    }
}
...
// https://chronicles.delimp.in/wp-admin/admin.php?page=betterdocs-admin